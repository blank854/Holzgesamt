...
router.post('/', authCheck, async(req, res, next) => {
    ...
    offer
        ...
        .catch((err) => {
            res.status(500).json({ message: "Beim Aktualisieren der Inserate ist ein Fehler aufgetreten.", error: err })
        })
})
...
router.put('/', authCheck, (req, res, next) => {
    ...
    Offer.findById(oID)
            ...
            if (offer.user === req.authUserData.userId) {
                ...
                Offer.findByIdAndUpdate(oID, offerUpdate)
                    ...
                    .catch((err) => {
                        res
                            .status(500)
                            .json({ message: "Beim Aktualisieren des Inserats ist ein Fehler aufgetreten.", error: err })
                    })
            } else {
        ...
        .catch((err) => {
            res.status(500).json({ message: "Beim Suchen des Inserats ist ein Fehler aufgetreten.", error: err })
        })
})
        ...
        .then((offerResult) => {
            try {
                ...
                console.log(
                    res
                    .status(500)
                    .json({ message: "Beim Suchen des Inserats ist ein Fehler aufgetreten.", error: err })
                )
            }
        })
        .catch((err) => {
...
router.delete('/:offerID', authCheck, (req, res, next) => {
    ...
    Offer.findByIdAndDelete(oID)
        ...
        .catch((err) => {
            res
                .status(500)
                .json({ message: "Beim Löschen des Inserats ist ein Fehler aufgetreten.", error: err })
        })
})
